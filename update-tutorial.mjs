...
const tutorialComponent = `export function Tutorial() {
  ...
  useEffect(() => {
    if (isMobile) {
      const seen = localStorage.getItem("hasSeenMobileTutorialv1");
      if (!seen) {
  ...
  const handleClose = () => {
    setShow(false);
    localStorage.setItem("hasSeenMobileTutorialv1", "true");
  };
      ...
      {show && (
        ...
        >
          <motion.div
            ...
            className="flex flex-col bg-[#0a0a0c] border-t border-white/10 rounded-t-3xl overflow-hidden shadow-2xl w-full"
