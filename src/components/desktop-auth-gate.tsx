...
export function DesktopAuthGate({ onEnterDemo: _onEnterDemo, onBackToLanding }: DesktopAuthGateProps) {
  ...
  const inputClass =
    "w-full rounded-xl bg-[#111] border border-[#1e1e1e] px-3.5 sm:px-4 py-2.5 sm:py-3 text-sm text-white placeholder:text-white/20 outline-none focus:border-[#FF6B00]/50 transition-colors";
          ...
          {mode === "complete-google" ? (
            <div className="px-5 sm:px-6 pt-5 pb-3 border-b border-[#111] text-center bg-white/[0.01]">
              ...
              <p className="text-[11px] text-white/40 mt-1 font-medium">
                Devam etmek için lütfen aşağıdaki eksik alanları doldurun.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={form.tcKimlik}
                      onChange={e =>
                        setForm(f => ({ ...f, tcKimlik: e.target.value.replace(/\D/g, "").slice(0, 11) }))
                      }
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      max={new Date(Date.now() - 18 * 365 * 864e5).toISOString().slice(0, 10)}
                    />
                  </div>
